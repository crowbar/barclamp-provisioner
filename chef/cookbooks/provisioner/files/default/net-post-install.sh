...
if [ -d "$rsyslog_dir" ]; then
	...
	if ! wget -O "$rsyslog_dir/00-crowbar-debug.conf" -q http://$HTTP_SERVER/rsyslog.d/00-crowbar-debug.conf
	then
...
if ! wget -O /target/root/.ssh/authorized_keys.wget -q http://$HTTP_SERVER/authorized_keys; then
...
wget -q http://$HTTP_SERVER/$OS_TOKEN/crowbar_join.sh -O- > /target/etc/init.d/crowbar_join.sh
